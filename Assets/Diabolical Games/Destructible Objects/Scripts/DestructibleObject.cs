...
{
            new Vector3(-1f, 0f, -1f), // vértice 1
            new Vector3(1f, 0f, -1f),  // vértice 2
            new Vector3(0f, 0f, 1f)    // vértice 3
};
            Vector3 basePosition = coinSpawnPoint != null
                ? coinSpawnPoint.position
                : transform.position;

            for (int i = 0; i < coinAmount; i++)
            {
                Vector3 spawnOffset = trianglePoints[i % trianglePoints.Length];
